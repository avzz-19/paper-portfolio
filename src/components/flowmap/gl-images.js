...
class TextureLoader extends Texture {
  ...
  stop() {
    if (this._gsap && typeof gsap !== "undefined" && gsap.ticker) {
    ...
    this.element && this.element.removeEventListener("mousemove", this.onMouseMove);
    if (this.renderer && this.renderer.gl && this.renderer.gl.canvas && this.renderer.gl.canvas.parentNode) {
      // Keep canvas, but could remove if needed
    }
  }
}
...
class GLImage extends GLContext {
  constructor(element) {
    ...
    const paper = new TextureLoader(this.gl).load("/images/paper-bg.jpg");
    ...
    // Animation loop: use gsap ticker if available, otherwise rAF
